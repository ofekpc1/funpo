...
namespace Nop.Plugin.Shipping.FixedByWeightByTotal.Infrastructure.Cache
{
    ...
    public partial class FixedByWeightByTotalEventConsumer : IConsumer<EntityDeletedEvent<ShippingMethod>>
    {
        ...
        public void HandleEvent(EntityDeletedEvent<ShippingMethod> eventMessage)
        {
            ...
            //delete saved free shipping status if exists
            var settingFreeShipping = _settingService.GetSetting(string.Format(FixedByWeightByTotalDefaults.FreeShippingSettingsKey, shippingMethod.Id));
            if (setting != null)
                _settingService.DeleteSetting(settingFreeShipping);
        }
