...
namespace Nop.Services.Shipping
{
    ...
    public partial class ShippingService : IShippingService
    {
        ...
        private readonly IWorkContext _workContext; //ADDED FOR FREE SHIPPING
        private readonly ISettingService _settingService; //ADDED FOR FREE SHIPPING
        #endregion
        ...
        public ShippingService(IAddressService addressService,
            ...
            ShoppingCartSettings shoppingCartSettings,
             IWorkContext workContext, //ADDED FOR FREE SHIPPING
             ISettingService settingService //ADDED FOR FREE SHIPPING
            )
        {
            ...
            this._workContext = workContext; //ADDED FOR FREE SHIPPING
            this._settingService = settingService; //ADDED FOR FREE SHIPPING
        }
        ...
        public virtual bool IsFreeShipping(ShoppingCartItem shoppingCartItem)
        {
            ...
            if (string.IsNullOrEmpty(shoppingCartItem.AttributesXml))
                //return true; //CHANGE FOR FREE SHIPPING
                status = true;  //ADDED FOR FREE SHIPPING

            //and whether associated products of the shopping cart item is free shipping
            //CHANGE FOR FREE SHIPPING
            status = _productAttributeParser.ParseProductAttributeValues(shoppingCartItem.AttributesXml)
                .Where(attributeValue => attributeValue.AttributeValueType == AttributeValueType.AssociatedToProduct)
            ...
            if (status)
            {
                ...
                {
                    //ADDED FOR FREE SHIPPING
                    var selectedShippingMethods = GetAllShippingMethods().Where(s => s.Name == shippingOption.Name).FirstOrDefault();

                    if (selectedShippingMethods != null)
                    {

                        bool isFreeShipping = _settingService.GetSettingByKey<bool>(string.Format("ShippingRateComputationMethod.FixedByWeightByTotal.FreeShipping.ShippingMethodId{0}", selectedShippingMethods.Id));
                        var shipppingMethodRate = _settingService.GetSettingByKey<decimal>(string.Format("ShippingRateComputationMethod.FixedByWeightByTotal.Rate.ShippingMethodId{0}", selectedShippingMethods.Id));
